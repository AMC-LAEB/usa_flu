...
generateFullXML <- function(clusterdata,season,subtype){
  ...
  if (!(dir.exists(paste0("beast_xmls/",season,"_",subtype)))){
    dir.create(paste0("beast_xmls/",season,"_",subtype))
  }
  
  cat(str,file=paste0("beast_xmls/",season,"_",subtype,'/',season,"_",subtype,'.xml'))
...
getTreeData <- function(cluster,subtype=NA,season=NA){
  ...
  taxa_to_keep = c()
  mtdt_subsampled = read.csv("/Users/simondejong/US_phylo/subsampled_mtdt_phylo.tsv",h=T,sep=',')
...
for (i in 1:length(seasons)){
  ...
  for (subtype in c("H3N2","H1N1","Vic","Yam")){
    ...
    generateFullXML(l,season,subtype)
    generateBEASTGeoXML_HHS (l,season,subtype)
