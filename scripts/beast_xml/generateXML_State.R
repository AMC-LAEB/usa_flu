generateBEASTGeoXML <- function(clusterdata,season,subtype){
  ...
  generateDTA <- function(names, predictor_str){
	...
	<generalSubstitutionModel id="tree1.state.model" randomizeIndicator="false">
		...
		<frequencies>
			<frequencyModel id="tree1.state.frequencyModel" normalize="true">
				<generalDataType idref="tree1.state.dataType"/>
				<frequencies>
					<parameter id="tree1.state.frequencies" dimension="49"/>
				</frequencies>
			</frequencyModel>
		</frequencies>
		...
		<rates>
			<parameter id="tree1.state.rates" dimension="2352" value="1.0" lower="0.0"/>
		</rates>
		<rateIndicator>
			<parameter id="tree1.state.indicators" dimension="2352" value="1.0"/>
		</rateIndicator>
	...
	<markovJumpsTreeLikelihood id="tree1.state.treeLikelihood" stateTagName="state" useUniformization="true" saveCompleteHistory="true" logCompleteHistory="true">
		...
		<frequencyModel id="tree1.state.root.frequencyModel" normalize="true">
			<generalDataType idref="tree1.state.dataType"/>
			<frequencies>
				<parameter id="tree1.state.root.frequencies" dimension="49"/>
			</frequencies>
		</frequencyModel>
...
<column label="tree1.age(root)" sf="6" width="12">
    ...
    str = paste0(str,'</log>
               <log id="fileLog" logEvery="5000" fileName="',subtype,"_",season,'.geo.log" overwrite="false">
			<joint idref="joint"/>
			<prior idref="prior"/>
			<likelihood idref="likelihood"/>
')
...
<log id="tree1.completeJumpHistory" logEvery="10000" fileName="tree1.geo.jumpHistory.log">
			<completeHistoryLogger>
				<markovJumpsTreeLikelihood idref="tree1.state.treeLikelihood"/>
			</completeHistoryLogger>
		</log>
		
		<logTree id="tree1.treeFileLog1" logEvery="10000" nexusFormat="true" fileName="tree1.geo.trees" sortTranslationTable="true">
			<treeModel idref="tree1.treeModel"/>
			<joint idref="joint"/>
...
<logTree id="tree1.treeFileLog2" logEvery="10000" nexusFormat="true" fileName="tree1.geo.history.trees" sortTranslationTable="true">
			<treeModel idref="tree1.treeModel"/>	
			<markovJumpsTreeLikelihood idref="tree1.state.treeLikelihood"/>
		</logTree>
							'
  ...
  generateXML <- function(clusterdata,season,subtype){
    ...
    cat(str,file=paste0("beast_xmls/",season,"_",subtype,'/',season,"_",subtype,'_geo.xml'))
  }
